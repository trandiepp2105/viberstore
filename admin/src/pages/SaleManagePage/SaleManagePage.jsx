...
const SaleManagePage = () => {
  const listSaleRef = useRef(null);
  const [distanceListProductToBottom, setDistanceListProductToBottom] =
    useState(0);

  useEffect(() => {
    const updatePosition = () => {
      if (listSaleRef.current) {
        const rect = listSaleRef.current.getBoundingClientRect();
  ...
  const [isOpenDeleteSalePopup, setIsOpenDeleteSalePopup] = useState(false);
  const handleToggleDeleteSalePopup = () => {
    setIsOpenDeleteSalePopup(!isOpenDeleteSalePopup);
  };
  return (
    <div className="user-page">
      {isOpenDeleteSalePopup && (
        <AcceptancePopup
          description="Are you sure you want to delete this sale promotion?"
          handleClose={handleToggleDeleteSalePopup}
        />
      )}
      <div className="page-content">
        <div className="header">
          <h3 className="title">Sale Promotions</h3>
